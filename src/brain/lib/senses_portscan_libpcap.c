...
*/
		...
		for ( loop = 0; loop < max; loop++) {
			...
			for ( i = cur_ip; i < cur_ip + MAXSCANADDRESS && i <= end_addr; i++, p++) {
				...
				if(repley!=2){
					if ( (p->status != 0) && 
							(p->status != 106) ) {
						p->status = scan(p->sock, i, port, result, &return_byte);
					...
					p->status = scan(p->sock, start_addr, target_port[i], result, &return_byte);
					my_sin.sin_addr.s_addr = htonl(start_addr);
					usleep(1000);
				}
	...
	{
		char st[1024];
		sprintf(st , "echo D#%sL%s > /tmp/read_sense3", (char*)inet_ntoa(my_sin.sin_addr), result);
		system(st);
		//fprintf(stdout, "%s\n", st);
	}
...
int scan(int sock, unsigned long ip, int port, char *result, int *return_byte ) {
	...
	if ( n == 0 ) {
		...
		*return_byte += sprintf(result+(*return_byte), "%dP", ntohs(sin.sin_port));
