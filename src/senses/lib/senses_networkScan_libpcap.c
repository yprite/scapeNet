...
int check_reply_packet(const unsigned char *packet, struct pcap_pkthdr *pkthdr, unsigned char *source_ip, NodeStatus *p_node_status, int pipeFd)
{
	...
	if (ntohs(arpheader->htype) == 1 && ntohs(arpheader->ptype) == 0x0800){ 
		...
		sprintf(result,"echo D~%d.%d.%d.%d > /tmp/read_sense", arpheader->spa[0], arpheader->spa[1], arpheader->spa[2], arpheader->spa[3]);
...
void confirmNodeTraffic(const unsigned char *packet, struct pcap_pkthdr *pkthdr, unsigned char *source_ip, int pipeFd)
{
	...
	if(ntohs(ether->h_proto) == 0x0800){
		...
		if(c_src_ip[0] == 210 && c_src_ip[1] == 118 && c_src_ip[2] == 34){
			sprintf(result,"echo D@%d.%d.%d.%dL%dLu%dL%d.%d.%d.%d > /tmp/read_sense2", c_src_ip[0], c_src_ip[1], c_src_ip[2], c_src_ip[3], ntohs(tcp->dest), pkthdr->len, c_dst_ip[0], c_dst_ip[1], c_dst_ip[2], c_dst_ip[3]);
			
			printf("echo D@%d.%d.%d.%dL%dLu%dL%d.%d.%d.%d\n", c_src_ip[0], c_src_ip[1], c_src_ip[2], c_src_ip[3], ntohs(tcp->dest), pkthdr->len, c_dst_ip[0], c_dst_ip[1], c_dst_ip[2], c_dst_ip[3]);
		// printf("%d ", *((u_char*)(&n_ip_temp)+i) );
		}
		else{
			sprintf(result,"echo D@%d.%d.%d.%dL%dLd%dL%d.%d.%d.%d > /tmp/read_sense2", c_dst_ip[0], c_dst_ip[1], c_dst_ip[2], c_dst_ip[3], ntohs(tcp->source), pkthdr->len, c_src_ip[0], c_src_ip[1], c_src_ip[2], c_src_ip[3]);
			printf("echo D@%d.%d.%d.%dL%dLd%dL%d.%d.%d.%d\n", c_dst_ip[0], c_dst_ip[1], c_dst_ip[2], c_dst_ip[3], ntohs(tcp->source), pkthdr->len, c_src_ip[0], c_src_ip[1], c_src_ip[2], c_src_ip[3]);
