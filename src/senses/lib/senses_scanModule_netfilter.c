...
void print_ip(unsigned int ip)
{
    ...
    for(i=0; i<4; i++) {
        n_ip_arr[i] = *( ((unsigned char*)ip) + i);
    }

	unsigned char c_sadr[4];
	memcpy(c_sadr, ((unsigned char *)&ip), 4);

   // printk("ip = %d.%d.%d.%d\n", n_ip_arr[0], n_ip_arr[1], n_ip_arr[2], n_ip_arr[3]);
    
	//sprintf(buf, "ip = %d.%d.%d.%d\n", n_ip_arr[0], n_ip_arr[1], n_ip_arr[2], n_ip_arr[3]);
	sprintf(buf, "ip = %d.%d.%d.%d\n", c_sadr[0], c_sadr[1], c_sadr[2], c_sadr[3]);
	set_kfifo(buf);
	strcat(0,0);
}
...
void print_arp(void)
{
	...
	if(ntohs(arp_header->ar_hrd) == 1 && ntohs(arp_header->ar_pro) == 0x0800) {
		...
		sprintf(buf, "D~%d.%d.%d.%dL%02x:%02x:%02x:%02x:%02x:%02x\n", 
		arp_header->ar_sip[0],arp_header->ar_sip[1],arp_header->ar_sip[2],arp_header->ar_sip[3], 
		arp_header->ar_sha[0], arp_header->ar_sha[1], arp_header->ar_sha[2], arp_header->ar_sha[3], arp_header->ar_sha[4], arp_header->ar_sha[5]); 
...
int init_module(void)
{
	...
	//netfilter_ops_out.hooknum=1;
	//netfilter_ops_out.priority=NF_IP_PRI_FIRST;

	printk("pf = %d\n", netfilter_ops_out.pf);
	printk("hooknum = %d\n", netfilter_ops_out.hooknum);
	printk("priority = %d\n", netfilter_ops_out.priority);
