...
/*
 * 모든 PORT를 SCAN할 경우 상당히 시간이 많이 걸리기 때문에 
 * 실제로 사용되는 PORT만 선정해서 SCAN
 */
...
/** @brief port scan을 위한 함수
 *  @param arg target에 대한 정보들
 */
...
/** @brief 실제로 Scan을 진행하는 함수.
 *  @param sock 연결에 사용할 Socket
 *  @param ip target의 IP
 *  @param port 스캔할 대상 Port
 *  @return scan한 결과에 대한 갯수
 */
